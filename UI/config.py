...
class defineWindow(QWidget):
    def __init__(self,parent = None):
        ...
        try:
            self.itemlist = eval(PARSER.get("config", "searchlable"))
        except:
            QMessageBox.warning(self,"Tips","config.ini配置文件config-searchlable列表异常，请检查配置文件",QMessageBox.Yes)
            self.itemlist = []
        self.list = []
        readconfig = Rwfile()
    ...
    def configMainWindow(self):
        ...
        self.combox1.addItems( self.itemlist)
        ...
        main_layout.addWidget(self.middle_widget,770)
    ...
    def setlbtnlayout(self):
        ...
        for btn in self.btnlist:
            if width < 5:
                ...
                width = 0
                height += 1
            self.middle_layout.addWidget(btn, height, width, 1, 1)
            width += 1
        if height <= 19:
            self.middle_layout.addWidget(QFrame(self), height + 1, 0, 20 - height - 1, 5)
...
class ConfigWindow(QWidget):
    ...
    def configWindow(self):
        ...
        try:
            indexlist = eval(PARSER.get("config","searchlable"))
        except:
            QMessageBox.warning(self, "Tips", "config.ini配置文件config-searchlable列表异常，请检查配置文件", QMessageBox.Yes)
            indexlist = []
        self.combox1.addItems(indexlist)
...
class SearchAndShowconfig(QWidget):
    def __init__(self,configdict,queryName):
        ...
        #索引日期连接符号
        self.index_date_connect = PARSER.get("config","index_data_con")
        # #日期连接符号
        # self.data_connect_char  = PARSER.get("config","data_connect")
    ...
    def getAllIndex(self):
        ...
        print("getAllIndex-body:{}".format(body))
        self.response=self.getResponse(body,index,days=1)
        lock = threading.Lock()
        for i in range(10):
    ...
    def getResponse(self, body, index, days=7):
        if isinstance(index, list):
            print(f"idnexlist:{index}")
            days = len(index)
            index = index[0][:-11]
        es_new = Elasticsearch_5601(body,index,days = days)
        return es_new.getdata()
    ...
    def getsearchindex(self,index,days=7):
        ...
        if index != "setup_log":
            for i in range(0, days):
                totalDays.append((today - datetime.timedelta(i)).__format__('%Y-%m-%d'))
        else:
            for i in range(0, 100):
                totalDays.append((today - datetime.timedelta(i)).__format__('%Y.%m.%d'))
        for i in totalDays:
            totalIndex.append(index + self.index_date_connect+ i)
        return totalIndex
    ...
    def transform_to_datetime(self,string):
        ...
        try:
            for s in string.split("\n"):
                ...
                if s != "":
                    time += str(datetime.datetime.utcfromtimestamp(int(s)))+"\n"
                else:
                    time += "\n"
            time = time[:-1]
        except:
    ...
    def searchResultByConditions(self):
        ...
        if self.searchResult["hits"]["total"] == 0 and "setup" in index[0]:
            ...
            for a in index:
                s.append(a.replace(".",""))
            # print("s:{}".format(s))
            # print('self.configdict[self.text]["body"]:{}'.format(self.configdict[self.text]["body"]))
